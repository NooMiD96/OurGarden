...
[ApiController]
public class OrderController(IOurGardenRepository repository, ILogger<OrderController> logger) : BaseController
{
    ...
    {
        var orders = await repository.GetOrders(includeProductInfo: true);
        var statusList = await repository.GetStatusList();
    ...
    public async Task<IActionResult> Update([FromBody]OrderUpdateDTO orderDTO)
    {
        ...
        try
        {
            var order = await repository.GetOrder(orderDTO.OrderId);
            ...
            var status = await repository.GetStatus(orderDTO.StatusId);
            if (status is null)
            {
                return LogBadRequest(
                    logger,
