...
[ApiController]
public class ProductController(IOurGardenRepository repository, ILogger<ProductController> logger, IPhotoSaver photoSaver, IPhotoEntityUpdater photoEntityUpdater) : BaseController
{
    private readonly ProductControllerService _service = new ProductControllerService(repository, logger, photoSaver, photoEntityUpdater);
    ...
    {
        var result = await repository.GetSimpleCategories(includeSubcategory: true);
    ...
    public async Task<IActionResult> AddOrUpdate([FromForm]ProductDTO productDTO)
    {
        ...
        try
        {
            ...
            {
                var oldProduct = await repository.GetProduct(productDTO.CategoryId, productDTO.SubcategoryId, productDTO.ProductId);
