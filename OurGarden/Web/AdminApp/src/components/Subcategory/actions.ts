...
export const actionsList = {
  ...
  addOrUpdateSubcategorySuccess: (
    payload: boolean
  ): t.IAddOrUpdateSubcategorySuccess => ({
    type: t.ADD_OR_UPDATE_SUBCATEGORY_SUCCESS,
    payload
  }),
  addOrUpdateSubcategoryError: (
    errorMessage: string
  ): t.IAddOrUpdateSubcategoryError => ({
...
export const actionCreators = {
  ...
  RemoveSubcategory: (
    ...
    t.IDeleteSubcategory | t.TGetSubcategoryList | t.ICleanErrorInnerAction
  > => (dispatch, getState) => {
    const apiUrl = "Delete";
    const xptToHeader = GetXsrfToHeader(getState);
    ...
    const fetchTask = fetch(
      `/${apiPrefix}/${controllerName}/${apiUrl}?categoryId=${categoryId}&subcategoryId=${subcategoryId}`,
      {
        credentials: "same-origin",
        method: "POST",
        headers: {
