...
export const actionCreators = {
  getOrderList: (): IAppThunkAction<
    t.TGetOrderList | t.ICleanErrorInnerAction
  > => (dispatch, getState) => {
    const apiUrl = "GetAll";
    ...
    const fetchTask = fetch(`/api/${controllerName}/${apiUrl}`, {
      ...
      .then(
        (
          value: IResponse<{ orders: IOrder[]; statusList: IOrderStatus[] }>
        ) => {
  ...
  updateOrder: (
    ...
    t.TAddOrUpdateOrder | t.TGetOrderList | t.ICleanErrorInnerAction
  > => (dispatch, getState) => {
    const apiUrl = "Update";
    const xptToHeader = GetXsrfToHeader(getState);
  ...
  removeOrder: (
    ...
    t.IDeleteOrder | t.TGetOrderList | t.ICleanErrorInnerAction
  > => (dispatch, getState) => {
    const apiUrl = "Delete";
    const xptToHeader = GetXsrfToHeader(getState);
    ...
    const fetchTask = fetch(
      `/api/${controllerName}/${apiUrl}?orderId=${orderId}`,
      {
        credentials: "same-origin",
        method: "POST",
        headers: {
